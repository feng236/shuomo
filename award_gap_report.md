...
2. 第四问储能配置已按参考答案口径改为最大弃电场景口径。
   - 第 4(1) 先保留现有 `40MW` 风电、`64MW` 光伏下的自给性分析。
   - 先识别 24 个风光组合中弃电量最大的场景，即 `W4P1`。
   - 储能容量扫描和主推荐基于 `W4P1`，再把推荐容量放回 24 个场景中复核。

3. 第四问储能方案不再只给单点答案，而给出多准则设计建议。
   - 输出表：`python/outputs/tables/q4_storage_design_recommendations.csv`
   - 主推荐采用“最大弃电场景下新增吨氨储能成本最低”的经济型方案。
   - 最大日产量、最大弃电削减和 90% 弃电削减最小容量作为敏感性对照。
...
2. 第四问必须明确区分“最大弃电场景”和“最坏场景”。
   - 主线按最大弃电场景 `W4P1` 配置储能，回答储能消纳弃电、提升产量的工程价值。
   - minimax 扩容结果作为鲁棒性对照，不作为当前储能主推荐口径。
   - 论文正文要写清楚选择 `W4P1` 的理由：它是弃电最多、最能体现储能消纳收益的场景。
